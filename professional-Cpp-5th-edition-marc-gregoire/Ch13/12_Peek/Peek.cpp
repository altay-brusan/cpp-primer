/*
 * Chapter 13 - peek(): Looking Ahead
 *
 * peek() previews the next character without removing it from the stream - like
 * looking up the chute without letting a value fall down. This is the third
 * variant of getReservationData(): it peeks at each character to decide whether
 * to keep reading the name or stop at the first digit, instead of reading then
 * unget()ing as the previous sample did.
 */
