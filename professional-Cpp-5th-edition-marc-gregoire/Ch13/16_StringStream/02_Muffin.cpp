/*
 * Chapter 13 - String Streams: Parsing with istringstream (Marshaling)
 *
 * The input side of string streams. createMuffin() reads a fixed-format string
 * from an std::istringstream (the 'i' is for input), pulling out the quoted
 * description, size, and a boolean chips flag with chained >> and the quoted /
 * boolalpha manipulators. Turning text back into an object this way is a form
 * of marshaling. A std::formatter<Muffin> handles the output side.
 */
