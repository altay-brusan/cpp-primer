/*
 * Chapter 19 - The not_fn Negator
 *
 * Demonstrates std::not_fn (C++17), a negator adapter that wraps a callable and returns
 * the logical complement of its result. Here not_fn(intEqual) turns the equality matcher
 * into an "not equal" predicate, so findMatches() reports positions where the two spans
 * differ rather than where they match.
 */
#include <print>
#include <vector>
#include <functional>
#include <span>

using namespace std;

template<predicate<int, int> Matcher, invocable<size_t, int, int> MatchHandler>
void findMatches(span<const int> values1, span<const int> values2,
	Matcher matcher, MatchHandler handler)
{
	if (values1.size() != values2.size()) { return; } // Must be same size.

	for (size_t i{ 0 }; i < values1.size(); ++i) {
		if (matcher(values1[i], values2[i])) {
			handler(i, values1[i], values2[i]);
		}
	}
}

bool intEqual(int value1, int value2)
{
	return value1 == value2;
}

bool bothOdd(int value1, int value2)
{
	return value1 % 2 == 1 && value2 % 2 == 1;
}

void printMatch(size_t position, int value1, int value2)
{
	println("Match found at position {} ({}, {})", position, value1, value2);
}

int main()
{
	vector values1{ 2, 5, 6, 9, 10, 1, 1 };
	vector values2{ 4, 4, 2, 9, 0, 3, 1 };
	println("Calling findMatches() using not_fn(intEqual):");
	findMatches(values1, values2, not_fn(intEqual), printMatch);
}
