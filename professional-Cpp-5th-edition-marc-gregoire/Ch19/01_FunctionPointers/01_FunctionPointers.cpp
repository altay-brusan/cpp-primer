/*
 * Chapter 19 - Function Pointers as Callbacks
 *
 * Demonstrates classic C-style function pointers used as callbacks. Two type aliases
 * declare pointer-to-function types: Matcher = bool(*)(int, int) and
 * MatchHandler = void(*)(size_t, int, int). The higher-order function findMatches()
 * accepts both as parameters and calls them just like regular functions, walking two
 * spans in parallel. Passing intEqual vs bothOdd as the Matcher shows how one generic
 * routine is customized purely by the function pointer handed to it.
 *
 * Key notes:
 *   - Function names decay to their address, so the & before a function name is optional.
 *   - findMatches() requires both spans to have the same size.
 */
#include <print>
#include <vector>
#include <span>
