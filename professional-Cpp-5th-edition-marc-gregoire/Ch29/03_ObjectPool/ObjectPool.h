/*
 * Chapter 29 - Object Pool Class Template (Design-Level Efficiency)
 *
 * An object pool that recycles short-lived objects to avoid repeated trips to the
 * memory manager. It allocates memory in growing chunks (each chunk double the
 * previous, like std::vector) of uninitialized storage via an allocator, and tracks
 * a free list of T pointers. acquireObject() constructs a T in place with placement
 * new, perfectly forwarding any arguments, and returns a shared_ptr whose custom
 * deleter destroys the object with std::destroy_at and returns the slot to the free
 * list instead of freeing memory. The pool only grows until destroyed, and is not
 * thread-safe. This technique pays off most for objects with large data members.
 *
 * Key notes:
 *   - Header-only class template: include it; there is no separate .cpp to compile.
 *   - The pool must outlive every object it hands out.
 *   - std::launder is used after placement new to obtain a valid pointer to the new
 *     object.
 */
