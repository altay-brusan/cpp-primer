/*
 * Chapter 29 - NameDB Third Attempt (Profiling)
 *
 * Implementation for NameDB.h - see the header for the conceptual notes.
 * The constructor body reduces to "while (inputFile >> name) m_names[name] += 1;",
 * collapsing the separate exists-check and insert into one map access per name.
 * getNameRank() and getAbsoluteNumber() are unchanged from the second attempt.
 */
