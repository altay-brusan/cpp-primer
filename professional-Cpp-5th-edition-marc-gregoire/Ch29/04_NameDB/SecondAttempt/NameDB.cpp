/*
 * Chapter 29 - NameDB Second Attempt (Profiling)
 *
 * Implementation for NameDB.h - see the header for the conceptual notes.
 * The constructor reads names and calls incrementIfExists(); only when that returns
 * false (a brand-new name) does it call addNewName(). All map lookups are O(log n),
 * yielding a large speedup over the vector version. getNameRank() still scans the full
 * map to count higher-frequency names, which remains an opportunity for further tuning.
 */
