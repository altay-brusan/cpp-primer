/*
 * Chapter 29 - NameDB First Attempt (Profiling)
 *
 * Implementation for NameDB.h - see the header for the conceptual notes.
 * nameExists(), incrementNameCount(), and getAbsoluteNumber() each loop over the
 * whole vector, so populating the database from a large file performs huge numbers of
 * linear searches. The loops are written out explicitly (rather than using algorithms)
 * to make the performance problem obvious in a profiler's output.
 */
