/*
 * Chapter 30 - The Code Under Test: a Generic Object Pool
 *
 * This is the production class that the chapter's unit tests exercise, not the test code
 * itself. ObjectPool<T> hands out reusable T instances backed by chunks of raw, allocator-
 * provided memory: acquireObject() placement-news a T into a free slot and returns a
 * std::shared_ptr whose custom deleter destroys the object and returns its memory to the
 * free list once the reference count hits zero. The pool only grows, doubling each new
 * chunk. Having a self-contained, well-specified component like this is exactly what makes
 * code testable - each behavior (creation, acquisition, uniqueness, reuse) maps to a test.
 *
 * Key notes:
 *   - Uses placement new plus std::launder and std::destroy_at to manage object lifetimes
 *     within preallocated storage, separating allocation from construction.
 *   - Copy and move are deleted, so the pool has a single, clear ownership model.
 *   - The destructor asserts that every handed-out object was returned before destruction.
 */
#pragma once
