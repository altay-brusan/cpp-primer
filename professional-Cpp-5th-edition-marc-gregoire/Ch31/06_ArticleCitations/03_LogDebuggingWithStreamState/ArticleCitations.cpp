/*
 * Chapter 31 - Article Citations (Stage 3: Logging the Stream State)
 *
 * Implementation for ArticleCitations.h - see the header for the conceptual notes.
 * Deepens the log-based debugging by adding a printStreamState() helper that reports good, bad,
 * fail, and eof after each read. The traces show the stream goes to eof then fail at the end of the
 * counting pass and stays in fail during the re-read, exposing the root cause: seekg() does not
 * clear the error state, so clear() must be called before seeking back.
 */
