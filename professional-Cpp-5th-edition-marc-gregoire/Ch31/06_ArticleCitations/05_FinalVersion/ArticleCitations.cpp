/*
 * Chapter 31 - Article Citations (Stage 5: Final, Corrected Version)
 *
 * Implementation for ArticleCitations.h - see the header for the conceptual notes.
 * The fully debugged version. After a symbolic debugger (GDB or Visual C++) traced a bad_alloc to
 * copy() allocating new string[m_numCitations] with m_numCitations == -1, readFile() now frees any
 * previously allocated array, resets the pointer to nullptr, and initializes m_numCitations to 0
 * before counting. Combined with the earlier clear() fix, the no-citations case and re-reads both work.
 */
