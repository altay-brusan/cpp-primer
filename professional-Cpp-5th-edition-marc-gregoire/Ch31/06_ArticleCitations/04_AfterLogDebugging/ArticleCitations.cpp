/*
 * Chapter 31 - Article Citations (Stage 4: After the Stream-State Fix)
 *
 * Implementation for ArticleCitations.h - see the header for the conceptual notes.
 * The trace messages are removed and the stream-state bug is fixed: readFile() now calls
 * inputFile.clear() before seekg() so the citations are re-read correctly. One bug still remains -
 * a file with no citations sets m_numCitations to -1, which crashes later when copy() allocates a
 * negative-sized array; this is the defect chased down with a debugger in the next stage.
 */
