/*
 * Chapter 31 - Article Citations (Stage 2: Log-Based Debugging)
 *
 * Implementation for ArticleCitations.h - see the header for the conceptual notes.
 * Same buggy logic as stage 1, now instrumented with println() trace messages inside readFile()
 * to pinpoint the failure. The traces reveal that the counting pass finds all four citations, yet
 * the second reading pass prints nothing between "reading citations" and "finished" - narrowing the
 * bug to the re-read after the seekg().
 */
