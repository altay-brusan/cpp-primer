/*
 * Chapter 31 - Article Citations (Stage 1: Buggy First Attempt)
 *
 * Implementation for ArticleCitations.h - see the header for the conceptual notes.
 * This is the original buggy version that the rest of the example debugs. readFile() counts the
 * citations by reading to EOF, but never calls clear() before seekg(), so the stream stays in a
 * failed state and the second pass reads nothing; it also sets m_numCitations to -1 (instead of 0)
 * when a file has no citations, which later makes copy() try to allocate a negative-sized array.
 */
