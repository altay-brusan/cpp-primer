 ...
 *
 * Implementation for RingBuffer.h - see the header for the conceptual notes.
 * addStringEntry() drops each message into the slot at m_next, advances, and wraps back to the
 * start (setting m_wrapped) when it reaches the end. operator<< prints the wrapped tail first
 * (from m_next to end) then the head (begin to m_next), reconstructing chronological order.
 */
