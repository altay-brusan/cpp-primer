/*
 * Chapter 25 - Writing a Custom Iterator (over the graph's nodes)
 *
 * A hand-written iterator that lets the directed_graph be traversed like any Standard Library
 * container. It models a bidirectional iterator, so it supplies the five required iterator-trait
 * aliases (value_type, difference_type, iterator_category, pointer, reference), a default
 * constructor, dereference (operator* and operator->), and pre/post increment and decrement.
 * Internally it just wraps the underlying node-vector const_iterator and exposes node values
 * rather than the internal graph_node objects.
 *
 * Key notes:
 *   - Used as both iterator and const_iterator: the graph exposes only read-only access to values.
 *   - operator== is defaulted (C++20), which compares the wrapped node iterators.
 */
