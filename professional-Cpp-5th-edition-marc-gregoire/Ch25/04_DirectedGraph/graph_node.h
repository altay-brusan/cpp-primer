/*
 * Chapter 25 - Custom Container Building Block: the Node (full directed_graph)
 *
 * The per-node helper for the full, iterator-enabled directed_graph. Identical in spirit to the
 * basic version: each graph_node holds one value plus an adjacency list of indices into the
 * graph's node vector, and remove_node_index() shifts the surviving indices down when a node is
 * removed. Keeping relationships as indices (instead of pointers) keeps them valid across vector
 * reallocations.
 *
 * Key notes:
 *   - graph_node is an internal detail (in the details namespace), not part of the public API.
 *   - The set is rebuilt after shifting because std::set elements cannot be modified in place.
 */
