/*
 * Chapter 25 - A Fully Standard Library-Compatible Custom Container (directed_graph)
 *
 * The complete, STL-conforming version of the directed_graph. Beyond the basic container it
 * publishes the full set of container type aliases (value_type, reference, size_type, iterator,
 * const_iterator, reverse_iterator, and so on) and the conventional interface: range and
 * initializer_list constructors, assign(), insert() overloads (including a hint and an
 * insert_range()), iterator-based and value-based erase(), at()/operator[], and begin()/end() in
 * forward, const, and reverse forms (the latter built from std::reverse_iterator). These wire the
 * custom iterators in so the graph works with range-based for and algorithms like find/count_if.
 *
 * Key notes:
 *   - nodes_adjacent_to() returns an optional range object exposing the adjacent-nodes iterators,
 *     usable directly in a range-based for loop.
 *   - reverse iterators come for free by adapting the forward iterators with std::reverse_iterator.
 *   - The iterator class is befriended so erase() can reach the wrapped underlying node iterator.
 */
