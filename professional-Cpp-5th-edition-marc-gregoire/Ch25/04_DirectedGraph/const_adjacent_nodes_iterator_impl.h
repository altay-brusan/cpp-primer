/*
 * Chapter 25 - Writing a Second Custom Iterator (over a node's neighbors)
 *
 * A separate bidirectional iterator that walks the nodes adjacent to a given node. The graph stores
 * adjacency as a set of indices, so this iterator wraps a std::set<size_t>::const_iterator together
 * with a back-pointer to the graph, and on dereference it translates the current index into the
 * actual node value via the graph's operator[]. A default-constructed instance (null graph pointer)
 * acts as the end iterator. Together with nodes_adjacent_to(), this lets neighbors be traversed
 * with range-based for and Standard Library algorithms.
 *
 * Key notes:
 *   - operator-> returns a pointer to the looked-up value so the compiler can re-apply -> to it.
 *   - operator== treats two iterators with null graph pointers as equal (both end iterators).
 */
