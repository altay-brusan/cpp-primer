/*
 * Chapter 25 - Exercising the Standard Library-Compatible Container
 *
 * Driver that demonstrates the full directed_graph behaving like a real Standard Library container.
 * It uses class template argument deduction and an initializer_list constructor, insert_range(),
 * structured bindings on insert()'s pair return, forward/const/reverse iteration (member and
 * non-member begin/end), range-based for, the standard find() and count_if() algorithms, all the
 * erase() overloads (single, range, by value), adjacency-range traversal via nodes_adjacent_to(),
 * plus size/max_size/empty, swap, copy, order-independent equality, and assignment.
 *
 * Key notes:
 *   - "template class directed_graph<string>;" forces full instantiation so every member compiles.
 */
