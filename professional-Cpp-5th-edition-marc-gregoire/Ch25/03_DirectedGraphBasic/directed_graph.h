/*
 * Chapter 25 - Writing a Custom Container (basic directed_graph)
 *
 * A first, deliberately minimal implementation of a custom STL-style container that models a
 * directed graph. Nodes are kept in a std::vector of graph_node helpers, and edges are stored as
 * sets of node indices. This basic version provides the core operations (insert, erase, edges,
 * lookup, equality, swap, size, adjacency queries) but does NOT yet expose iterators, so it cannot
 * be used with range-based for or the Standard Library algorithms. The next sample (04_DirectedGraph)
 * adds custom iterators to make it fully Standard Library-compatible.
 *
 * Key notes:
 *   - Equality compares node sets and their adjacency value sets, independent of insertion order.
 *   - operator= uses the copy-and-swap idiom (via the standalone swap()) for strong exception safety.
 */
