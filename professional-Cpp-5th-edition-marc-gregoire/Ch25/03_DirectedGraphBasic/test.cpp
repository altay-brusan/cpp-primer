/*
 * Chapter 25 - Exercising the Basic Custom Container
 *
 * Driver that puts the basic directed_graph through its paces: inserting nodes and edges, erasing
 * an edge and a node, printing in DOT format, swapping, copying, testing order-independent
 * equality, assignment, and using the container with a non-trivial element type (std::string).
 * The explicit "template class directed_graph<string>;" forces the whole template to instantiate
 * so every member is type-checked even if main() does not call it.
 */
