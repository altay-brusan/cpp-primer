/*
 * Chapter 15 - Overloading the Subscripting Operator
 *
 * A dynamically growing array template that overloads operator[]. Two overloads are
 * provided: a non-const T& operator[](size_t) that grows the backing buffer on demand (so
 * it can serve as an lvalue, e.g. myArray[i] = 100), and a const T& operator[](size_t)
 * const that gives read-only access and throws std::out_of_range instead of growing. This
 * is why the read-only overload can be selected only on const objects - you cannot
 * overload on return type alone.
 *
 * Key notes:
 *   - operator[] takes exactly one index; chaining Array<Array<int>> a; a[2][4] is how
 *     multidimensional access is faked with single-index subscripting.
 *   - A commented-out alternative const operator[] returns a static zero-initialized
