/*
 * Chapter 15 - Overloading User-Defined Literal Operators
 *
 * Defines cooked-mode user-defined literals. operator""_km and operator""_m build a Length
 * (stored in meters) from a long double literal, e.g. 1.2_km; they are friends of Length so
 * they can use its private constructor. operator""_s builds a std::string from a string
 * literal via the (const char*, size_t) cooked form, e.g. "Hello World"_s.
 *
 * Key notes:
 *   - Cooked operators receive the interpreted value (long double, or char array + length);
 *     a commented-out raw _m operator taking const char* shows the alternative raw mode.
 *   - User-defined literal suffixes must start with an underscore (standard suffixes such
 *     as s/sv do not).
 */
#include <print>
#include <string>
