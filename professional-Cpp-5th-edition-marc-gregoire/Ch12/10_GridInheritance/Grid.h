/*
 * Chapter 12 - Inheritance From a Class Template (base)
 *
 * Same `Grid<T>` template as before. The companion `GameBoard<T>` extends
 * it with a `move()` member while inheriting Grid's constructors. The
 * point of the sample is the syntactic gotchas around inheriting from a
 * template - see GameBoard.h for the details.
 */
#pragma once
