/*
 * Chapter 12 - Template Specialization (primary template)
 *
 * The general-purpose `Grid<T>`. The companion header `GridString.h`
 * defines a *full specialization* for `T = const char*` that stores
 * `std::string` internally instead of `const char*`, so the grid actually
 * owns its strings.
 *
 * The trick is invisible to clients: `Grid<const char*>` looks like the
 * primary template at the call site, but uses a completely different
