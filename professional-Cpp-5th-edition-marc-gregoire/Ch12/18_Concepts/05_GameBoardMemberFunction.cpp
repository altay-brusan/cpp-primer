/*
 * Chapter 12 - Concepts (5/6): Constraining a Member Function
 *
 * Building on sample 04: `GameBoard<T>::move` only makes sense if T is
 * moveable. The previous version assumed it. Here `move` carries its own
 * `requires std::movable<T>` clause:
 *
 *     void move(...) requires std::movable<T>;
 *
 * Now if you instantiate `GameBoard<NonMovableType>`, the class still
 * exists but trying to *call* `move` is a compile error. That's exactly
 * the right granularity - only the operation that needs the property is
 * restricted, not the whole class.
 *
 * Key notes:
 *   - The constraint goes on BOTH the declaration (inside the class) and
 *     the out-of-class definition. They must match.
 *   - `std::movable` is composed of several smaller concepts
 *     (`move_constructible`, `assignable_from<T&, T>`, `swappable`) - the
 *     concept name documents intent without you having to spell out the
 *     mechanics.
 */
#include <cstddef>
#include <vector>
