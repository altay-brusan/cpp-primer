/*
 * Chapter 22 - Durations
 *
 * Introduces std::chrono::duration, which represents a time span as a count of
 * ticks together with a ratio giving the tick period in seconds. This sample
 * builds custom durations (for example ratio<60> for minutes), compares and does
 * arithmetic on them, and converts between tick periods. Converting to a coarser
 * period that would lose precision requires an explicit duration_cast, while
 * conversions that lose nothing (or use a floating-point representation) are
 * implicit.
 *
 * Key notes:
 *   - Predefined durations include hours, minutes, seconds, milliseconds, etc.
 *   - C++ provides standard duration literals such as 42min and 30s.
 */
