/*
 * Chapter 22 - Dates and the C++20 Calendar
 *
 * A tour of the C++20 calendar types: year, month, and day, and the composite
 * year_month_day, plus partial types such as month_day, year_month_day_last, and
 * year_month_weekday_last (for example "the last Monday of June 2020"). Dates can
 * be built fluently with the overloaded operator/ and with literals like 2020y,
 * June, and 22d. The sample distinguishes "serial" types (sys_days, a time_point
 * counted in days) from "field-based" types (year_month_day) and converts between
 * them with floor<days> and sys_days.
 *
 * Key notes:
 *   - Calendar values can be written to streams and to std::format/println, with
 *     {:L} and chrono format specs like %A applying the current locale.
 *   - Arithmetic differs by type: adding years to a serial time_point shifts by a
 *     fixed number of seconds, whereas adding years to a field-based year_month_day
 *     keeps the same calendar month and day.
 */
