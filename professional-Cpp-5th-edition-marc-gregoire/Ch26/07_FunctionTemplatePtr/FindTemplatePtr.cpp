/*
 * Chapter 26 - Emulating Function Partial Specialization with Overloading
 *
 * C++ does not allow partial specialization of function templates, so instead
 * the Find() function template is overloaded. There is the generic Find(), an
 * overload taking pointer arguments (T* value, T* const* arr) that dereferences
 * before comparing, and a non-template overload for const char* that uses
 * strcmp. The compiler picks the most specific viable candidate at each call.
 *
 * Key notes:
 *   - When equally specific, a non-template overload is preferred over a
 *     function-template instantiation.
 *   - The printed tags ("original", "overload", "ptr overload") reveal which
 *     overload deduction selected for each call in main().
 */
