/*
 * Chapter 26 - Multiple Template Type Parameters Constrained by a Concept
 *
 * Extends the Grid class template with a second template type parameter that
 * selects the underlying container (e.g. vector or deque). The container is
 * constrained by the GridContainerType concept, which requires it to be a
 * random-access range that exposes a value_type alias and a resize() method,
 * so misuse fails with a clear constraint error instead of a cryptic one.
 *
 * Key notes:
 *   - The element type is reached through Container::value_type, not T directly.
 *   - Passing a non-conforming type such as double does not satisfy the concept.
 */
