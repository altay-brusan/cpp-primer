/*
 * Chapter 26 - Template Template Parameters
 *
 * Instead of taking a fully-formed container type, this Grid takes a container
 * template itself (such as vector or deque) as its second parameter. This lets
 * clients write Grid<int, vector> rather than Grid<int, vector<optional<int>>>,
 * avoiding the repetition of the element type. The Grid supplies the element
 * type by instantiating the template as Container<optional<T>> internally.
 *
 * Key notes:
 *   - A template template parameter mirrors the parameter list of the template
 *     it accepts: template <typename E, typename Allocator> class Container.
 *   - Since C++17 the keyword typename may be used in place of class here.
 */
