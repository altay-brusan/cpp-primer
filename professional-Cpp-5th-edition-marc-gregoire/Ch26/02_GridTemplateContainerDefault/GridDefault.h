/*
 * Chapter 26 - Default Value for a Template Type Parameter
 *
 * Same concept-constrained Grid as before, but the Container parameter now has
 * a default argument of vector<optional<T>>. Just like default function
 * arguments, a default template argument lets clients omit the second
 * parameter; note that T from the first parameter is reused inside the default.
 *
 * Key notes:
 *   - The default value is written only on the class template header, never
 *     repeated on the out-of-class method definitions.
 */
