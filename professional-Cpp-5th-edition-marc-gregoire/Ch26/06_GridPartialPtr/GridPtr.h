/*
 * Chapter 26 - Partial Specialization for All Pointer Types
 *
 * Specializes Grid for any pointer element type via Grid<U*>, storing the data
 * as unique_ptr<U> so the grid owns the pointed-to objects. The copy
 * constructor performs a deep copy and the copy assignment operator uses the
 * copy-and-swap idiom, requiring a noexcept swap() method.
 *
 * Key notes:
 *   - When a client writes Grid<int*>, the parameter U deduces to int, not int*.
 *   - A single specialization covers an entire family of types (all pointers),
 *     which is the real power of partial specialization.
 */
