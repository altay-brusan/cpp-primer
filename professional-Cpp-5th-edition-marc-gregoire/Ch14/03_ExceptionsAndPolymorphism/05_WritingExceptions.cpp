/*
 * Chapter 14 - Writing Your Own Exception Class Hierarchy
 *
 * Defines a custom hierarchy rooted at FileError, which derives from
 * std::exception and overrides what(). FileOpenError and FileReadError derive
 * from FileError, building tailored messages (FileReadError also stores the
 * line number). readIntegerFile() throws the specific type, and main() catches
 * the whole family with a single catch (const FileError&). Custom classes give
 * meaningful names and let you attach extra data beyond a plain message.
 *
 * Key notes:
 *   - what() is overridden as noexcept and returns a const char* valid for the
 *     object's lifetime (backed by the m_message member).
 *   - Exception objects must be copyable/movable, since a thrown object is
 *     copied or moved at least once.
 */
