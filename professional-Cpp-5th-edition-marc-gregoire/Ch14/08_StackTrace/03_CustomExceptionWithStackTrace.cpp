/*
 * Chapter 14 - Custom Exception Carrying Source Location and Stack Trace
 *
 * Combines source_location and stacktrace inside a custom exception. MyException
 * captures both the throw site (source_location::current()) and the call stack
 * (stacktrace::current(1), skipping the constructor's own top frame) as default
 * constructor arguments. main() catches it and, via the where() and how()
 * accessors, prints the location plus each frame's description, source_file(),
 * and source_line() - a self-documenting exception for debugging.
 */
