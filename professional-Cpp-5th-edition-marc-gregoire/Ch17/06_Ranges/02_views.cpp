/*
 * Chapter 17 - Views and Range Adapters
 *
 * Walks through the core range adapters applied one at a time to a vector: filtering
 * (views::filter), transforming to another type (views::transform), dropping leading
 * elements (views::drop), and reversing (views::reverse), each built with the pipe
 * operator and printed via printRange(). Views are lazy and non-owning, so each
 * intermediate result1..result4 does no work until iterated. The second half
 * showcases C++23 adapters: views::zip, adjacent<2>, chunk, stride, split with
 * enumerate, as_rvalue, and cartesian_product.
 *
 * Key notes:
 *   - Requires a C++23 standard library; many adapters here (zip, adjacent, chunk,
 *     stride, enumerate, as_rvalue, cartesian_product) are not available in C++20.
 */
