/*
 * Chapter 17 - Converting Ranges to Containers (ranges::to)
 *
 * Demonstrates the C++23 ranges::to facility for materializing a range or view into a
 * concrete container. Shows ranges::to<set>(vec) with deduced element type, the pipe
 * form vec | ranges::to<set<double>>() with element conversion, and the equivalent
 * from_range constructor. It also converts a lazy iota/transform pipeline into a
 * vector<double>, and splits a string into words that are collected into a
 * vector<string>. Finally it exercises the container append_range() and
 * insert_range() member functions that consume a range directly.
 *
 * Key notes:
 *   - ranges::to, from_range, and the xyz_range() member functions are C++23
 *     features; requires a C++23 standard library.
 */
