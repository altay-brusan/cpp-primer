/*
 * Chapter 18 - std::array Basics
 *
 * std::array is a fixed-size sequential container that lives on the stack and
 * wraps a C-style array, while adding size(), iterators, and fill(). It takes
 * two template parameters: the element type and the compile-time element count.
 * The example reports size(), iterates the elements, and overwrites them all
 * with fill().
 */
