/*
 * Chapter 18 - Implementation for CableCompany.h - see the header for the conceptual notes.
 *
 * Implements the channel-subscription logic. Packages and customers live in
 * map<string, bitset> members; set() and reset() toggle individual channel
 * bits, and operator|= ors a package's channels into a customer's. The
 * non-const helper forwards to the const getCustomerChannels() through
 * const_cast to avoid duplicating the lookup.
 */
