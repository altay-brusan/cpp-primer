/*
 * Chapter 18 - bitset Example: Cable Company Channels
 *
 * Combines two containers: each cable package and each customer is a
 * bitset<NumChannels> (a set bit means "subscribed to that channel"), and both
 * are stored in map<string, bitset> members keyed by name. Declares operations
 * to manage packages, customers, and individual channels; implemented in
 * CableCompany.cpp.
 *
 * Key notes:
 *   - Channel sets combine with bitset's bitwise operators, e.g. |= to add a
 *     whole package to a customer.
 */
