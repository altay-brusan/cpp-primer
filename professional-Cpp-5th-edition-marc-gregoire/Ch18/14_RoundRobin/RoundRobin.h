/*
 * Chapter 18 - vector Example: RoundRobin Class Template
 *
 * A reusable wrapper around a vector that hands out its elements in a repeating
 * cycle - the classic round-robin scheduling pattern. add() appends, remove()
 * deletes the first matching element, and getNext() returns the next element,
 * wrapping back to the front after the last.
 *
 * Key notes:
 *   - The constructor calls reserve() to preallocate the caller's expected
 *     element count, avoiding reallocations.
 *   - The crux of the class is iterator invalidation: push_back() and erase()
 *     can invalidate m_nextElement, so each saves its position as a ptrdiff_t
 *     offset from begin() and rebuilds the iterator afterward.
 *   - Copy is deleted and move defaulted because the m_nextElement iterator
 *     would not survive a naive copy.
 *   - Header-only template; included and exercised by RoundRobinTest.cpp.
 */
