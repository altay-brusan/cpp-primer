/*
 * Chapter 18 - Implementation for BuddyList.h - see the header for the conceptual notes.
 *
 * Implements the multimap-based buddy lists. removeBuddy() uses lower_bound()
 * and upper_bound() to bound the range of entries sharing a key, while
 * isBuddy() and getBuddies() use equal_range() with structured bindings.
 * getBuddies() builds the result with the ranges library (views::values and
 * ranges::to).
 */
