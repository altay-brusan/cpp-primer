 ...
 * Chapter 18 - std::span
 *
 * std::span (C++20) is a lightweight, non-owning view over a contiguous
 * sequence - just a pointer and a length - so one print(span<const int>)
 * function accepts a vector, a std::array, or a C-style array, in whole or as a
 * subview. subspan() carves out a sub-range; a span is cheap and passed by
 * value, copying no data.
 *
 * Key notes:
 *   - span<const int> gives a read-only view; a non-const span can modify the
