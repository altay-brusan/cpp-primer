/*
 * Chapter 18 - queue Example: Network Packet Buffer
 *
 * A class template that buffers incoming packets in FIFO order using a
 * std::queue, the container adapter that provides first-in first-out semantics.
 * bufferPacket() enqueues with push() (dropping packets when an optional max
 * size is reached), and getNextPacket() retrieves with front() then removes
 * with pop().
 *
 * Key notes:
 *   - queue::pop() does not return the removed element, so getNextPacket()
 *     reads it with front() first.
 *   - Header-only template; included and exercised by PacketBufferTest.cpp.
 */
