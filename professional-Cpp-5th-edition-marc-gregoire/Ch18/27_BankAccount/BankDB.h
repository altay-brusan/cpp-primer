/*
 * Chapter 18 - map Example: Bank Account Database
 *
 * Models a bank account database with a map<int, BankAccount> keyed on the
 * account number - the common pattern of using one field of the stored object
 * as the key. Declares add, delete, find (by number and by name), and merge
 * operations; implementations are in BankDB.cpp.
 *
 * Key notes:
 *   - Lookup by the key (account number) is logarithmic via find(); lookup by a
 *     non-key field (client name) requires a linear search.
 */
