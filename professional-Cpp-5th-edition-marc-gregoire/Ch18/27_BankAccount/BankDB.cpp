/*
 * Chapter 18 - Implementation for BankDB.h - see the header for the conceptual notes.
 *
 * Implements the map-based account database: emplace() for keyed insertion that
 * reports success, find() for fast key lookup, a linear scan with structured
 * bindings for lookup by a non-key field, erase() by key, and merge() to absorb
 * another database. Commented-out alternatives show the pre-auto and
 * pre-structured-bindings equivalents.
 */
