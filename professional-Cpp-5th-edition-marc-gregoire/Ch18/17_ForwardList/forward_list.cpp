/*
 * Chapter 18 - forward_list
 *
 * std::forward_list is a singly linked list supporting forward iteration only,
 * so its mutating operations work "after" a position: splice_after(),
 * insert_after(), and the before_begin() iterator that denotes the slot before
 * the first element. The example merges three lists into one ordered sequence.
 *
 * Key notes:
 *   - There is no size() and no way to step backward, so reaching the last
 *     element requires iterating from before_begin().
 */
