/*
 * Chapter 20 - The merge Algorithm
 *
 * Demonstrates std::merge(), which combines two sorted source ranges into a single
 * sorted destination range in linear time. Both inputs are sorted first and the
 * destination is sized to hold the sum of their sizes.
 *
 * Key notes:
 *   - merge() is more efficient than concatenating and re-sorting (O(N) vs O(N log N)).
 */
