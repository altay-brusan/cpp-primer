/*
 * Chapter 20 - Set Algorithms on Sorted Ranges
 *
 * Demonstrates the set algorithms, which operate on sorted ranges: includes()
 * tests subset containment, while set_union(), set_intersection(),
 * set_difference(), and set_symmetric_difference() produce the corresponding
 * combinations into a destination range. Each returns the end of the written
 * result. A DumpRange() helper built on ranges::subrange prints the results.
 *
 * Key notes:
 *   - The result range must be pre-sized large enough; inputs are sorted first.
 */
