/*
 * Chapter 20 - The nth_element Selection Algorithm
 *
 * Demonstrates std::nth_element(), which places the element that would occupy the
 * nth position of a fully sorted range into that position, partitioning the rest
 * around it, all in linear time. The first use (with greater<>) finds the third
 * largest value; the second obtains the five largest, which are then sort()ed and
 * printed with for_each_n().
 */
