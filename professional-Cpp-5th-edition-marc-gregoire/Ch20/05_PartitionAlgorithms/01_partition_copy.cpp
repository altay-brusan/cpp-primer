/*
 * Chapter 20 - The partition_copy Algorithm
 *
 * Demonstrates std::partition_copy(), which copies each source element to one of
 * two destinations depending on whether a predicate returns true or false. Here
 * the input is split into even numbers and odd numbers. The returned pair of
 * iterators marks the end of each destination's copied region and is used with
 * erase() to trim the pre-sized destination vectors.
 */
