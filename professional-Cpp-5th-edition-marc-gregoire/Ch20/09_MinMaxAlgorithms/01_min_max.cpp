/*
 * Chapter 20 - Minimum/Maximum Algorithms
 *
 * Demonstrates the min/max family. min() and max() compare two or more values
 * (including an initializer_list) and minmax() returns both as a pair; structured
 * bindings unpack the pair. min_element()/max_element() and minmax_element() work
 * on iterator ranges and return iterators to the extreme elements.
 *
 * Key notes:
 *   - The bare min()/max() versions take values, not iterator ranges.
 */
