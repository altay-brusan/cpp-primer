/*
 * Chapter 20 - Range Comparison Algorithms
 *
 * Demonstrates comparing two ranges held in different container types (a vector
 * and a list). equal() reports whether all corresponding elements match;
 * mismatch() returns iterators to the first position where they differ; and
 * lexicographical_compare() orders the two ranges dictionary-style. The
 * four-iterator overloads of equal() and mismatch() are used here because they
 * safely handle ranges of differing lengths.
 *
 * Key notes:
 *   - A commented-out block preserves the older, less safe three-iterator forms.
 */
