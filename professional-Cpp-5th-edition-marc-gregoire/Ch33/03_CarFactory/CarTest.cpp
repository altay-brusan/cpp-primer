/*
 * Chapter 33 - Factory Method in Action
 *
 * Driver for the Factory Method pattern. It also defines LeastBusyFactory, itself
 * a CarFactory whose createCar() override forwards the request to whichever held
 * factory has produced the fewest cars so far - composing factories through the
 * same factory-method interface. main() requests cars from individual factories
 * and from the load-balancing factory to show the polymorphic creation at work.
 *
 * Key notes:
 *   - Contains main(); run as a standalone program.
 *   - LeastBusyFactory uses ranges::min_element over the production counts.
 */
