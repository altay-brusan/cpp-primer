/*
 * Chapter 33 - Observer Pattern
 *
 * Demonstrates the Observer pattern using a reusable, variadic Event<...> class.
 * Observers register callbacks with an Event and receive an EventHandle they can
 * later use to unregister. The ObservableSubject owns several typed events and
 * raises them when its data changes, notifying every registered observer without
 * knowing who they are. The Observer class shows the RAII idiom of registering in
 * its constructor and automatically unregistering in its destructor.
 *
 * Key notes:
 *   - Event<Args...> stores callbacks in a map keyed by handle for easy removal.
 *   - Observers can be free functions, lambdas, or member-function wrappers.
