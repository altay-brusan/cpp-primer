 ...
 * Compare against folder 04: cleanup() and moveFrom() are gone, replaced by a
 * one-line swap-based body in each move operation.
 */
...
// Move constructor: *this is default-initialized (null/zero members) before the
// body runs, so swapping with src transfers ownership without copying anything.
...
// Move assignment operator: take rhs into a temporary, swap with us, let the
// temporary die at end of scope (it now owns whatever *this used to hold and
