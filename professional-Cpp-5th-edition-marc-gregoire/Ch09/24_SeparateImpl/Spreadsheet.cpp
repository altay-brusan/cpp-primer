 ...
 * The whole `Spreadsheet::Impl` class is defined at the top of this file so it
 * never appears in the public header. The remaining definitions are forwarding
 * shims from Spreadsheet to its Impl.
 */
...
Spreadsheet::Spreadsheet(const Spreadsheet& src)
	// Dereference src.m_impl to call Impl's copy constructor, then wrap the
	// fresh Impl in a unique_ptr - unique_ptr itself is not copyable.
	: m_impl{ make_unique<Impl>(*src.m_impl) }
{
}

// These must be defaulted in this .cpp file, not in the header: Impl is only
// forward-declared in the header, and `unique_ptr<Impl>` cannot generate its
// destructor (or moves) without seeing the full type.
...
{
	// Dereferencing forces direct object-to-object assignment, which invokes
	// Impl's own (copy-and-swap based) operator=. Assigning the pointers would
	// just move ownership and leak the old buffer.
	*m_impl = *rhs.m_impl;
	return *this;
}
