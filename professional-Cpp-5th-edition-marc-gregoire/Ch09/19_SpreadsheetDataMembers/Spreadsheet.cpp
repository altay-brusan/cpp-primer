 ...
 * Notice that `ms_counter` is declared inline in the header, so the commented-out
 * `size_t Spreadsheet::ms_counter;` is unnecessary in C++17 and later.
 */
...
Spreadsheet::Spreadsheet(size_t width, size_t height,
	...
	: m_id{ ms_counter++ } // grabs the next id and bumps the shared counter
	, m_width{ min(width, MaxWidth) } // clamp to the class-scope maximum
	, m_height{ min(height, MaxHeight) }
	, m_theApp{ theApp } // reference member - must be initialized here
{
	println("{}", m_id);
